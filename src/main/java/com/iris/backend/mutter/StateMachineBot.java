...
implements Bot
{
	...
	public BotResponse respond(Session session, Context context, String messageText)
			throws BotException
	{
		log.info("utterance:"+messageText);
		log.info("session values at start:"+ session.toString());
		...
		try
		{
			...
			IntentMatch intentMatch = matcher.match(messageText, session, context, null, null);
			...
			log.info("session values at end:"+ session.toString());
			log.info("");
			log.info("");
			log.info("");
