...
public class StateMachine
{
	...
	public IntentResponse trigger(final IntentMatch intentMatched, final Session session)
	{
		...
		logger.info("current state name in state machine trigger function:"+currentStateName);
		...
		Intent intent = intentMatched.getIntent();
		String intentName = (intent != null) ? intent.getName() : null;

		if (intentName == null)
		{
			throw new IllegalArgumentException("Request missing intent." + intentMatched.toString());
		}
...
/*		// see if we have a pre-evet action and execute it if we have it
		...
		for (Transition transition : transitionToStateList)
		{
			...
			{
				if (transition.guard.passes(intentMatched, session))
				{
					transitionToState = transition.toState;
					break;
				}
		...
		logger.info("executing transition from:"+ currentState + " to "+ transitionToState);
		IntentResponse response = transitionToState.execute(intentMatched, session);
		logger.info("setting STATE_MACHINE_JLA1974_currentStateName:"+transitionToState.name);
