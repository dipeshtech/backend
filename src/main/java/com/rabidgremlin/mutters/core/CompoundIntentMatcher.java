    ...
    implements IntentMatcher
{
  ...
  public IntentMatch match(String utterance,Session session, Context context, Set<String> expectedIntents, HashMap<String, Object> debugValues)
  {
    ...
    IntentMatch match = firstMatcher.match(utterance,session, context, expectedIntents, debugValues);

    // no ? try second one
    if (match == null)
    {
      match = secondMatcher.match(utterance,session, context, expectedIntents, debugValues);
    }
